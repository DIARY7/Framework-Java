...
public class FrontController extends HttpServlet {
    ...
    HashMap<String,Mapping> dicoMapping;//Clé ny URL ,d mapping ny value
    @Override
    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse resp) throws IOException{
        ...
        // }
        for (int i = 0; i <dicoMapping.size() ; i++) {
                ...
                for (int i = 0; i < files.length; i++) {
                    if (files[i].getName().endsWith(".class")) {
                        ...
                        if (temp_class.getAnnotation(AnnotationCtrl.class)!=null) {
                            ...
                            setDicoMapping(temp_class, temp_name_classe); // Sprint 2
                        }
    ...
    /* sprint2 */
    public void setDicoMapping(Class c,String nameClass){
        Method[] methodes = c.getMethods();
        for (int j = 0; j < methodes.length; j++) {
            Get annotGet = methodes[j].getAnnotation(Get.class); 
            if ( annotGet !=null ) {
                dicoMapping.put(annotGet.url(), new Mapping( nameClass , methodes[j].getName())) ;
            }
        }
    }
